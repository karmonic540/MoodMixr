...
st.markdown(
    "<h1 style='text-align: center; color: #FF00FF;'>🎧 MoodMixr</h1>",
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    if file:
        ...
        def get_mood_icon(mood_text):
            ...
            if "happy" in mood_text or "uplift" in mood_text:
                return "😊"
            if "calm" in mood_text or "chill" in mood_text:
                return "😌"
            if "dark" in mood_text or "emotional" in mood_text:
                return "😢"
            if "energetic" in mood_text or "hype" in mood_text:
                return "🔥"
            return "🎧"
...
with tab2:
    ...
    files = st.file_uploader(
        "Upload multiple files", type=["mp3", "wav"], accept_multiple_files=True
    )
    ...
    if files:
        ...
        arc_display = " ➝ ".join(
            [f"{t['role']} ({t['filename'].split('.')[0][:10]})" for t in tracks]
        )
        st.markdown(
            f"<div style='padding:12px; background:#111; color:white; border-left:5px solid #FF00FF; border-radius:8px;'>🎛️ {arc_display}</div>",
            unsafe_allow_html=True,
        )
        ...
        for t in suggest_best_transitions(tracks):
            st.markdown(
                f"<div style='padding:10px; background:#111; margin:10px 0; border-left:4px solid #FF00FF;'><b>{t['from']} ➜ {t['to']}</b><br><small>{t['reason']}</small></div>",
                unsafe_allow_html=True,
            )

        df = pd.DataFrame(tracks)[["filename", "bpm", "key", "mood", "energy"]]
        st.download_button(
            "📥 Download CSV", df.to_csv(index=False).encode(), "dj_set.csv", "text/csv"
        )
...
with tab3:
    ...
    for i, col in enumerate(deck_cols):
        ...
        with col:
            ...
            uploaded_file = st.file_uploader(
                f"Upload Track for {deck_id}", type=["mp3", "wav"], key=f"deck{i}"
            )
            ...
            if uploaded_file:
                ...
                fig = generate_emotion_waveform(
                    y, sr, mood=mood, track_name=uploaded_file.name
                )
                st.pyplot(fig)
    ...
    # 🎚️ Compare Deck A & B
    if all(
        f"deck{idx}" in st.session_state and st.session_state[f"deck{idx}"]
        for idx in range(2)
    ):
        try:
            # Extract deck info
            ...
            if len(deck_data) == 2:
                ...
                energy_trend = (
                    "⬆️ Increasing" if b["energy"] > a["energy"] else "⬇️ Decreasing"
                )
...
with tab4:
    ...
    files = st.file_uploader(
        "Upload multiple tracks (.mp3/.wav)",
        type=["mp3", "wav"],
    ...
    if files:
        ...
        st.success(
            "✅ All tracks analyzed. You can now sort, download, or use this data for set prep."
        )
            ...
            for track in results:
                ...
                if preview_url:
                    ...
                    try:
                        ...
                        st.markdown(
                            f"BPM: **{bpm}** | Key: **{key}** | Energy: **{energy}%** | Mood: **{mood}**"
                        )
                    except Exception as e:
