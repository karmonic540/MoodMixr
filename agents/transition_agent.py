    ...
    def recommend(bpm, key, mood, energy):
        try:
            ...
            if energy >= 0.7:
                ...
                transitions.append(
                    "🎯 Maintain energy with similar vibe or tension builder"
                )
            else:
                transitions.append(
                    "🌙 Cooldown moment — use breakdown, ambient pad, or vocal cut"
                )
            ...
            if bpm < 85:
                ...
                transitions.append(
                    "💫 Transition into mid-tempo hip-hop, funk, or Afrobeat"
                )
            elif 110 <= bpm < 128:
                transitions.append(
                    "🔊 Groove into deep house, tech house, or melodic techno"
                )
            else:
            ...
            if mood in vibe_map:
                ...
                transitions.append(
                    f"🔍 Explore a {mood.lower()}-inspired transition layer"
                )

            # 🎼 Final tip
            transitions.append(
                f"🎼 Key: {key} — consider harmonic mixing or a 1-semitone jump for tension"
            )
